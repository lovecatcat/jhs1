<template>
  <div class="insurance">
        ...
        <div class="am-list-dropdown-list" v-if="insList">
          ...
          <app-select label="缴费年限" :readonly="mainPyAttr.length === 1">
            <select v-model.number='insurance.pay_year'
                    ...
                    @change="resetFee">
              <option disabled value=''>请选择</option>
              <option v-for="(item,index) in mainPyAttr" :value="item.pay_year" :key="index" v-if="payYearFilter(item.pay_year)">
          ...
          <template v-if="insurance.safe_id">
            ...
            <template v-if="isBaseMoney&&!fuBaseMoney&&insurance.safe_id !== '366'">
              ...
              <app-input label="基本保额"
                         v-if="insurance.safe_id !== '292' && insurance.safe_id !== '384'"
            ...
            <template v-else>
              ...
              <app-select label="生存金方式" v-if="insurance.safe_id === '379'">
                <!-- 恒大福享今生 -->
                <select v-model.number="flag['379']" @change="resetFee">
                  <option disabled value=''>请选择</option>
              ...
              <app-select label="保证年限" v-if="insurance.safe_id === '378'">
                <!-- 恒大福享今生 -->
                <select v-model.number="flag['378']" @change="resetFee">
                  <option disabled value=''>请选择</option>
    ...
    <app-dropdown
      ...
      <!-- 国华 附加少儿成长无忧年金保险 -->
      <template v-if="item.safe_id==='19389' && addonsSelected[item.safe_id]">
        <app-input label="份数">
          <input slot="input"
                 v-model.number="cache.derate_money19389"
                 type="number"
                 placeholder="请输入份数"
                 @change="addonsSelected[item.safe_id] = true, flagChanged(item.safe_id)">
          <div slot="icon" v-show="cache.derate_money19389 !== ''" class="am-list-clear"><i
            class="am-icon-clear am-icon" @click="cache.derate_money19389 = ''"></i></div>
        </app-input>
        <div class="am-list-item">
          <div class="am-list-content">保障期间</div>
          <div class="am-ft-black">至25周岁</div>
        </div>
        <div class="am-list-item">
          <div class="am-list-content">缴费期间</div>
          <div class="am-ft-black">10年</div>
        </div>
        <template v-if="addonRes[item.safe_id]">
          <div class="am-list-item">
            <div class="am-list-content">保险金额</div>
            <div class="am-ft-orange">{{10*insurance.period_money*cache.derate_money19389}}</div>
          </div>
          <div class="am-list-item">
      ...
      <template v-if="item.safe_id==='19392' && addonsSelected[item.safe_id]">
        <app-select label="保险金额">
          <select v-model.number="flag[item.safe_id]" @change="flagChanged(item.safe_id)">
            <option disabled value=''>请选择</option>
            <option value="15000">有社保5千元</option>
            <option value="25000">无社保5千元</option>
            <option value="110000">有社保1万元</option>
            <option value="210000">无社保1万元</option>
          </select>
        </app-select>
        <div class="am-list-item">
          <div class="am-list-content">保障期间</div>
      ...
      <template v-if="item.safe_id==='404' && addonsSelected[item.safe_id]">
        ...
        <div class="am-list-item">
          <div class="am-list-content">年交保费</div>
          <div class="am-ft-black">100元</div>
        </div>
      </template>
      <!-- 恒大传家宝-->
      <!-- 金掌柜年金保险 -->
      ...
      <template v-if="['388', '387', '386'].indexOf(item.safe_id) > -1 && addonRes[item.safe_id]">
        ...
        <div class="am-list-item">
          <div class="am-list-content">保险金额</div>
          <div class="am-ft-orange" v-if="addonRes[385]">{{(Number(insurance.period_money) + Number(addonRes[385]['年缴保费']))*(mainPayYear-1) | moneyFilter}}</div>
          <div class="am-ft-orange" v-else>{{insurance.period_money*(mainPayYear-1) | moneyFilter}}</div>
        </div>
        <div class="am-list-item">
          <div class="am-list-content">年缴保费</div>
          <div class="am-ft-orange">{{addonRes[item.safe_id]['年缴保费']}}</div>
      ...
      <!-- 国华康运金生 轻症疾病豁免保险费 -->
      <!-- 国华康运金生豁免保险费重大疾病保险（2017） -->

      <!-- 国华康运金生豁免保险费重大疾病保险（2017） -->
      <!-- 国华康运金生附加养老年金 -->
      <template v-if="item.safe_id === '385' && addonRes[item.safe_id]">
        <div class="am-list-item">
          <div class="am-list-content">保障期间</div>
          <div class="am-ft-black">至85周岁</div>
        </div>
        <div class="am-list-item">
          <div class="am-list-content">缴费期间</div>
        ...
        <div class="am-list-item">
          <div class="am-list-content">保险金额</div>
          <div class="am-ft-orange">{{insurance.period_money*mainPayYear | moneyFilter}}</div>
        </div>
        <div class="am-list-item">
          <div class="am-list-content">年缴保费</div>
          <div class="am-ft-orange">{{addonRes[item.safe_id]['年缴保费']}} </div>
      ...
      <!-- 国华康运金生附加养老年金 -->
      <!--  附加交通意外伤害保险 -->
      <template v-if="item.safe_id === '183' && addonsSelected[item.safe_id]">
        <app-input label="保险金额">
          <input slot="input"
                 v-model.number="cache.base_money183"
                 type="number"
                 placeholder="基本保险金额（元）"
                 @change="addonsSelected[item.safe_id] = true, flagChanged(item.safe_id)">
          <!-- @change="addonRes[item.safe_id]=null,addonsSelected[item.safe_id]=false"> -->
          <div slot="icon"
               v-show="cache.base_money183 != ''"
               @click="cache.base_money183 = ''"
               class="am-list-clear">
            <i class="am-icon-clear am-icon"></i>
          </div>
        </app-input>
        <app-select label="缴费期间">
        ...
        <app-select label="保险期间">
          <select v-model.number="flag[1831]"
            ...
            <option value="20">20年</option>
            <option value="30">30年</option>
            <option value="5500">至55岁</option>
            <option value="6000">至60岁</option>
            <option value="6500">至65岁</option>
            <option value="7000">至70岁</option>
            <option value="7500">至75岁</option>
          </select>
        </app-select>
      ...
      <!--  附加交通意外伤害保险 -->
      <!--  附加额外给付重大疾病保险（B款） -->
      <template v-if="item.safe_id === '383' && addonsSelected[item.safe_id]">
        <app-input label="保险金额">
          <input slot="input"
                 v-model.number="cache.base_money383"
                 type="number"
                 placeholder="基本保险金额（元）"
                 @change="addonsSelected[item.safe_id] = true, flagChanged(item.safe_id)">
          <!-- @change="addonRes[item.safe_id]=null,addonsSelected[item.safe_id]=false"> -->
          <div slot="icon"
               v-show="cache.base_money383 != ''"
               @click="cache.base_money383 = ''"
               class="am-list-clear">
            <i class="am-icon-clear am-icon"></i>
          </div>
        </app-input>
        <app-select label="缴费期间">
          <select v-model.number="flag[item.safe_id]"
            ...
            <option value="15">15年交</option>
            <option value="20">20年交</option>
            <option value="1800">交至18岁</option>
          </select>
        </app-select>
        <template v-if="addonRes[item.safe_id]">
          <div class="am-list-item">
      ...
      <!--  附加额外给付重大疾病保险（B款） -->
      <!-- 信泰附加投保人豁免保险费重大疾病保险 -->
      <template v-if="item.safe_id === '370' && addonRes[item.safe_id]">
        <div class="am-list-item">
          <div class="am-list-content">保障期间</div>
          <div class="am-ft-black">终身</div>
        </div>
...
<script>
  ...
  // 附加险上线产品
  const addonFilter = ['8', '11', '86', '94', '121', '131', '146', '147', '148', '175', '177', '196', '235', '236', '237', '273', '281', '284', '285', '289', '291', '293', '294', '295', '348', '380', '370', '383', '183', '385', '386', '387', '388', '19392', '19389']
  ...
  export default {
    ...
    methods: {
      ...
      insChanged () {
        ...
        } else if (safeid === '378') { // 恒大福享今生
          this.flag[safeid] = '' // 保证年限
        } else if (safeid === '379') { // 国华盛世鑫悦
          this.flag[safeid] = '' // 生存金方式
        }
      ...
      chAddonState (index) {
        ...
        if (this.addonsSelected[index]) {
          ...
          switch (index) {
            case '387': // 国华康运金生附加豁免轻症疾病豁免保险费
            case '388': // 国华康运金生附加豁免身故豁免保险费
              if (!this.addonsSelected[386]) {
                toastText = '请先完成豁免保险费重大疾病保险（2017）'
              }
              break
            case '386': // 国华康运金生附加豁免保险费重大疾病保险（2017）
              if (this.samePerson) {
              ...
              break
            case '385': // 国华康运金生附加养老年金
              if (this.mainPayYear === 30) {
                toastText = '主险30年交不可附加该险种'
              }
              break
            case '370': // 信泰附加投保人豁免保险费重大疾病保
              if (this.samePerson) {
                toastText = '投被保人为同人时不可附加该险种'
          ...
          if (mustSelected.indexOf(index) > -1) {
            ...
            this.$delete(this.addonInsData, 387)
            this.$delete(this.addonRes, 387)
            this.addonsSelected[387] = false
...
//            取消时 清除缓存的提交数据
      ...
      checkMainForm () {
        ...
        if (this.sc_id === '0') {
          ...
          toastText = '请选择主险领取年龄'
        } else if (safeid === '378' && !this.flag[safeid]) {
          toastText = '请选择主险保障年限'
        } else if (safeid === '379' && !this.flag[safeid]) {
      ...
      checkMainAge (safeid) {
        ...
        switch (safeid) {
          ...
          case '382': // 中英人寿金喜年年
            ...
            break
          case '381': // 中国人保鑫盈人生
            if (mainPayYear === 1 && assuAge > 70) {
              toastText = '趸交时被保人为年龄不能大于70周岁'
            } else if (mainPayYear === 3 && assuAge > 65) {
              toastText = '3年交被保人为年龄不能大于65周岁'
            } else if (mainPayYear === 5 && assuAge > 63) {
              toastText = '20年交被保人为年龄不能大于35周岁'
            } else if (mainPayYear === 10 && assuAge > 60) {
              toastText = '10年交被保人为年龄不能大于60周岁'
            }
            break
          case '379': // 国华盛世鑫悦
            if (mainPayYear === 1 && assuAge > 60 && this.flag[379] === 1) {
              toastText = '生存方式一且趸交时被保人年龄为28天-60周岁（含）'
            } else if (mainPayYear === 3 && assuAge > 57 && this.flag[379] === 1) {
              toastText = '生存方式一且3年交时被保人年龄为28天-57周岁（含）'
            } else if (mainPayYear === 5 && assuAge > 55 && this.flag[379] === 1) {
              toastText = '生存方式一且5年交时被保人年龄为28天-55周岁（含）'
            } else if (mainPayYear === 10 && assuAge > 50 && this.flag[379] === 1) {
              toastText = '生存方式一且10年交时被保人年龄为28天-50周岁（含）'
            } else if (mainPayYear === 1 && assuAge > 45 && this.flag[379] === 2) {
              toastText = '生存方式二且趸交时被保人年龄为28天-45周岁（含）'
            } else if (mainPayYear === 3 && assuAge > 42 && this.flag[379] === 2) {
              toastText = '生存方式二且3年交时被保人年龄为28天-42周岁（含）'
            } else if (mainPayYear === 5 && assuAge > 40 && this.flag[379] === 2) {
              toastText = '生存方式二且5年交时被保人年龄为28天-40周岁（含）'
            } else if (mainPayYear === 10 && assuAge > 35 && this.flag[379] === 2) {
              toastText = '生存方式二且10年交时被保人年龄为28天-35周岁（含）'
            } else if (mainPayYear === 1 && assuAge > 45 && this.flag[379] === 3) {
              toastText = '生存方式三且趸交时被保人年龄为18周岁-45周岁（含）'
            } else if (mainPayYear === 3 && assuAge > 42 && this.flag[379] === 3) {
              toastText = '生存方式三且3年交时被保人年龄为18周岁-42周岁（含）'
            } else if (mainPayYear === 5 && assuAge > 40 && this.flag[379] === 3) {
              toastText = '生存方式三且5年交时被保人年龄为18周岁-40周岁（含）'
            } else if (mainPayYear === 10 && assuAge > 35 && this.flag[379] === 3) {
              toastText = '生存方式三且10年交时被保人年龄为18周岁-35周岁（含）'
            } else if (mainPayYear === 1 && assuAge < 18 && this.flag[379] === 3) {
              toastText = '生存方式三且趸交时被保人年龄为18周岁-45周岁（含）'
            } else if (mainPayYear === 3 && assuAge < 18 && this.flag[379] === 3) {
              toastText = '生存方式三且3年交时被保人年龄为18周岁-42周岁（含）'
            } else if (mainPayYear === 5 && assuAge < 18 && this.flag[379] === 3) {
              toastText = '生存方式三且5年交时被保人年龄为18周岁-40周岁（含）'
            } else if (mainPayYear === 10 && assuAge < 18 && this.flag[379] === 3) {
              toastText = '生存方式三且10年交时被保人年龄为18周岁-35周岁（含）'
            }
            break
          case '378': // 恒大福享今生
      ...
      checkMainMoney (safeid) {
        ...
        switch (safeid) {
          case '19388': // 恒大万年红
            if (money < 300000) {
              toastText = '【' + name + '】最低基本保额为30万'
            } else if (money % 10000 !== 0) {
              toastText = '【' + name + '】保费需为1万元整数倍'
            }
            break
          case '401': // 恒大万年红
            if (periodMoney < 1000) {
      ...
      checkMainFee (safeid) {
        ...
        } // 379和377要禁止改判断不然不能计算保险金额，分别写在更改附加险和checkIns里面
        if (toastText) {
          this.$toast.open(toastText)
          return false
        }
        ...
        switch (safeid) {
          ...
          case '183':
            if (!this.cache.base_money183) {
              toastText = '请先输入保险金额'
            } else if (this.cache.base_money183 < 50000 && assuAge < 18) {
              toastText = '未成年人保险金额最低为5万元'
            } else if (this.cache.base_money183 < 100000 && assuAge >= 18) {
              toastText = '成年人保险金额最低为10万元'
            } else if (this.cache.base_money183 > 5000000 && assuAge >= 18) {
              toastText = '成年人保额上限为 500 万'
            } else if (this.cache.base_money183 > 200000 && assuAge < 10) {
              toastText = '0-9 岁未成年人保额上限为 20 万'
            } else if (this.cache.base_money183 > 500000 && assuAge >= 10 && assuAge < 18) {
              toastText = '10-17 岁未成年人保额上限为 50 万'
            } else if (this.cache.base_money183 % 10000 !== 0) {
            ...
            } else if (this.flag[1831] === 6000 && assuAge > 35 && flag === 20) {
            ...
            } else if (this.flag[1831] === 7500 && assuAge > 50 && flag === 20) {
          ...
          case '383': // 中英人寿附加额外给付重大疾病保险（B 款
            if (!this.cache.base_money383) {
              toastText = '请先输入保险金额'
            } else if (this.cache.base_money383 < 10000) {
              toastText = '保险金额最低为一万元'
            } else if (this.cache.base_money383 > 500000 && assuAge >= 18) {
              toastText = '成年人保额上限为 50 万'
            } else if (this.cache.base_money383 > 300000 && assuAge < 18) {
              toastText = '未成年人保额上限为 30 万'
            } else if (this.cache.base_money383 % 1000 !== 0) {
      ...
      calMoney (isMain, addonSafeid = 0) {
        ...
        // 险种参数
        if (safeid === '19389') { // 国华附加少儿成长无忧年金保险
          data.pay_year = 10
          data.safe_year = 2500
